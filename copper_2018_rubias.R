...
# setwd("C:/Users/krshedd/R/Copper River Inseason/2018/")
...
names(SW27_IDs) <- sillys

# Pool
PoolCollections.GCL(collections = sillys, loci = loci96, IDs = SW27_IDs, newname = sillys_strata)
SCDVTF18_SW27.gcl$n ## 190
table(SCDVTF18_SW27.gcl$attributes$CAPTURE_DATE)

strata_date_range <- unique(SCDVTF18_SW27.gcl$attributes$CAPTURE_DATE)
...
#### Data QC ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
sillys_strata_2018_SW27_n <- matrix(data = NA, nrow = length(sillys_strata), ncol = 4, 
                                    dimnames = list(sillys_strata, c("Genotyped", "Missing", "Duplicate", "Final")))
...
### Initial
## Get number of individuals per silly before removing missing loci individuals
sillys_strata_2018_SW27_n[, "Genotyped"] <- sapply(paste0(sillys_strata, ".gcl"), function(x) get(x)$n)

### Missing
## Remove individuals with >20% missing data
sillys_strata_2018_SW27_MissLoci <- RemoveIndMissLoci.GCL(sillyvec = sillys_strata, proportion = 0.8)
save_objects(objects = "sillys_strata_2018_SW27_MissLoci", path = "Objects")

## Get number of individuals per silly after removing missing loci individuals
sillys_strata_2018_SW27_n[, "Missing"] <- sillys_strata_2018_SW27_n[, "Genotyped"] - 
  sapply(paste0(sillys_strata, ".gcl"), function(x) get(x)$n)

### Duplicate
## Check within collections for duplicate individuals.
sillys_strata_2018_SW27_DuplicateCheck95MinProportion <- CheckDupWithinSilly.GCL(sillyvec = sillys_strata, loci = loci96, quantile = NULL, minproportion = 0.95)
detach("package:reshape", unload = TRUE); library(tidyverse)
sillys_strata_2018_SW27_DuplicateCheckReportSummary <- sapply(sillys_strata, function(x) sillys_strata_2018_SW27_DuplicateCheck95MinProportion[[x]]$report)
sillys_strata_2018_SW27_DuplicateCheckReportSummary
save_objects(objects = "sillys_strata_2018_SW27_DuplicateCheckReportSummary", path = "Objects")

## Remove duplicate individuals
sillys_strata_2018_SW27_RemovedDups <- RemoveDups.GCL(sillys_strata_2018_SW27_DuplicateCheck95MinProportion)

### Final
sillys_strata_2018_SW27_n[, "Final"] <- sapply(paste0(sillys_strata, ".gcl"), function(x) get(x)$n)
## Get number of individuals per silly after removing duplicate individuals
sillys_strata_2018_SW27_n[, "Duplicate"] <- sillys_strata_2018_SW27_n[, "Genotyped"] - sillys_strata_2018_SW27_n[, "Missing"] - sillys_strata_2018_SW27_n[, "Final"]
sillys_strata_2018_SW27_n

save_objects(objects = "sillys_strata_2018_SW27_n", path = "Objects")
...
# load_objects(path = "rubias/mixture")
str(copper_2018_SW27_loci89.mix, give.attr = FALSE)
...
# load_objects(path = "rubias/mixture")
str(copper_2018_SW27_loci91_v2.mix, give.attr = FALSE)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Run MSA
copper_2018_SW27.out <- run_rubias_mixture(reference = copper46_9groups_loci91.base,
                                           mixture = copper_2018_SW27.mix, 
                                           group_names = PWSCopper9Groups_46Pops_pub,
                                           gen_start_col = 5, 
                                           method = "MCMC", 
                                           reps = 25000, 
                                           ...
                                           sample_int_Pi = 10,
                                           path = "rubias/output")
str(copper_2018_SW27.out, give.attr = FALSE, max.level = 2)
# save.image("copper_2018_SW27.RData")
# load("rubias/output/copper_2018_SW27.RData")
copper_2018_SW27.out$mixing_proportions %>% 
  group_by(repunit) %>% 
  summarize(rho = sum(pi))

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## rubais output
copper_2018_SW27.sum <- custom_combine_rubias_output(rubias_output = copper_2018_SW27.out, group_names = PWSCopper9Groups_46Pops_pub, bias_corr = FALSE)
str(copper_2018_SW27.sum)
...
# Write out as .csv file
# dir.create("Estimates tables")
copper_2018_SW27_dates.sum <- copper_2018_SW27.sum %>%
  separate(col = mixture_collection, into = c("year", "stat_week"), sep = "_SW") %>%  # split mixture into silly and stat_week
  separate(col = year, into = c("trash", "year"), sep = "SCDVTF") %>%
  select(-trash) %>%
  mutate(year = as.integer(year) + 2000) %>%  # year as integer
...
save_objects(objects = "copper_2018_SW27_dates.sum", path = "Estimates objects")
write_csv(x = copper_2018_SW27_dates.sum, path = "Estimates tables/CopperRiver_Sockeye_2018_SW27_Estimates.csv")
...
#### Data QC ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
sillys_strata_2018_SW28_n <- matrix(data = NA, nrow = length(sillys_strata), ncol = 4, 
                                    dimnames = list(sillys_strata, c("Genotyped", "Missing", "Duplicate", "Final")))
...
### Initial
## Get number of individuals per silly before removing missing loci individuals
sillys_strata_2018_SW28_n[, "Genotyped"] <- sapply(paste0(sillys_strata, ".gcl"), function(x) get(x)$n)

### Missing
## Remove individuals with >20% missing data
sillys_strata_2018_SW28_MissLoci <- RemoveIndMissLoci.GCL(sillyvec = sillys_strata, proportion = 0.8)
save_objects(objects = "sillys_strata_2018_SW28_MissLoci", path = "Objects")

## Get number of individuals per silly after removing missing loci individuals
sillys_strata_2018_SW28_n[, "Missing"] <- sillys_strata_2018_SW28_n[, "Genotyped"] - 
  sapply(paste0(sillys_strata, ".gcl"), function(x) get(x)$n)

### Duplicate
## Check within collections for duplicate individuals.
sillys_strata_2018_SW28_DuplicateCheck95MinProportion <- CheckDupWithinSilly.GCL(sillyvec = sillys_strata, loci = loci96, quantile = NULL, minproportion = 0.95)
detach("package:reshape", unload = TRUE); library(tidyverse)
sillys_strata_2018_SW28_DuplicateCheckReportSummary <- sapply(sillys_strata, function(x) sillys_strata_2018_SW28_DuplicateCheck95MinProportion[[x]]$report)
sillys_strata_2018_SW28_DuplicateCheckReportSummary
save_objects(objects = "sillys_strata_2018_SW28_DuplicateCheckReportSummary", path = "Objects")

## Remove duplicate individuals
sillys_strata_2018_SW28_RemovedDups <- RemoveDups.GCL(sillys_strata_2018_SW28_DuplicateCheck95MinProportion)

### Final
sillys_strata_2018_SW28_n[, "Final"] <- sapply(paste0(sillys_strata, ".gcl"), function(x) get(x)$n)
## Get number of individuals per silly after removing duplicate individuals
sillys_strata_2018_SW28_n[, "Duplicate"] <- sillys_strata_2018_SW28_n[, "Genotyped"] - sillys_strata_2018_SW28_n[, "Missing"] - sillys_strata_2018_SW28_n[, "Final"]
sillys_strata_2018_SW28_n

save_objects(objects = "sillys_strata_2018_SW28_n", path = "Objects")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Create mixtures
copper_2018_SW28_loci89.mix <- create_rubias_mixture(sillyvec = sillys_strata, loci = loci89, path = "rubias/mixture/kma473")
save_objects(objects = "copper_2018_SW28_loci89.mix", path = "rubias/mixture/kma473")
# load_objects(path = "rubias/mixture")
str(copper_2018_SW28_loci89.mix, give.attr = FALSE)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Run MSA
copper_2018_SW28_loci89.out <- run_rubias_mixture(reference = kma473_loci89.base,
                                                  mixture = copper_2018_SW28_loci89.mix, 
                                                  group_names = Groups15,
                                                  gen_start_col = 5, 
                                                  method = "MCMC", 
                                                  reps = 25000, 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Create mixtures
copper_2018_SW28_loci91_v2.mix <- create_rubias_mixture(sillyvec = sillys_strata, loci = loci91_v2, path = "rubias/mixture/coastwide482")
save_objects(objects = "copper_2018_SW28_loci91_v2.mix", path = "rubias/mixture/coastwide482")
# load_objects(path = "rubias/mixture")
str(copper_2018_SW28_loci91_v2.mix, give.attr = FALSE)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Run MSA
copper_2018_SW28_loci91_v2.out <- run_rubias_mixture(reference = copper482_loci91_v2.base,
                                                     mixture = copper_2018_SW28_loci91_v2.mix, 
                                                     group_names = PWSCopper9Groups_pub,
                                                     gen_start_col = 5, 
                                                     method = "MCMC", 
                                                     reps = 25000, 
                                                     ...
                                                     sample_int_Pi = 10,
                                                     path = "rubias/output/coastwide482")
copper_2018_SW28_loci91_v2.out$mixing_proportions %>% 
  group_by(repunit) %>% 
  summarize(rho = sum(pi))  # Other = 2%
...
## Create mixtures
# dir.create("rubias/mixture")
copper_2018_SW28.mix <- create_rubias_mixture(sillyvec = sillys_strata, loci = loci91, path = "rubias/mixture")
save_objects(objects = "copper_2018_SW28.mix", path = "rubias/mixture")
# load_objects(path = "rubias/mixture")
str(copper_2018_SW28.mix, give.attr = FALSE)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Run MSA
copper_2018_SW28.out <- run_rubias_mixture(reference = copper46_9groups_loci91.base,
                                           mixture = copper_2018_SW28.mix, 
                                           group_names = PWSCopper9Groups_46Pops_pub,
                                           gen_start_col = 5, 
                                           method = "MCMC", 
                                           reps = 25000, 
                                           ...
                                           sample_int_Pi = 10,
                                           path = "rubias/output")
str(copper_2018_SW28.out, give.attr = FALSE, max.level = 2)
# save.image("copper_2018_SW28.RData")
# load("rubias/output/copper_2018_SW28.RData")
copper_2018_SW28.out$mixing_proportions %>% 
  group_by(repunit) %>% 
  summarize(rho = sum(pi))

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## rubais output
copper_2018_SW28.sum <- custom_combine_rubias_output(rubias_output = copper_2018_SW28.out, group_names = PWSCopper9Groups_46Pops_pub, bias_corr = FALSE)
str(copper_2018_SW28.sum)
...
# Write out as .csv file
# dir.create("Estimates tables")
copper_2018_SW28_dates.sum <- copper_2018_SW28.sum %>%
  separate(col = mixture_collection, into = c("year", "stat_week"), sep = "_SW") %>%  # split mixture into silly and stat_week
  separate(col = year, into = c("trash", "year"), sep = "SCDVF") %>%
  select(-trash) %>%
  mutate(year = as.integer(year) + 2000) %>%  # year as integer
  mutate(sample_date = paste(month(strata_date_range), day(strata_date_range), sep = "/")) %>%  # sample date
  rename(reporting_group = repunit) %>%  # rename to reporting group
...
save_objects(objects = "copper_2018_SW28_dates.sum", path = "Estimates objects")
write_csv(x = copper_2018_SW28_dates.sum, path = "Estimates tables/CopperRiver_Sockeye_2018_SW28_Estimates.csv")
